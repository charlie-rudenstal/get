...
function parse(tokens, data, callback) {
	...
	switch(token.type) {
		case 'SOURCE':
			...
			var params = [];
			var i = 1;
			while(i < tokens.length &&  tokens[i].type == "SYMBOL") {
				params += " " + tokens[i].text;
				i++;
			}
		...
		case 'SYMBOL':
			...
			// continue
			parse(tokens.splice(1), data, callback);
		...
		case 'MERGE':
			...
			// get next token to merge with
			parse(tokens.slice(1), data, function(newData) {
